...
5. **TONO Y COMIENZO:** Mantén un tono formal, estratégico e imperativo en las tareas. Inicia directamente con el texto del reporte, sin saludos ni introducciones previas.
      ...
      // --- FASE DE GENERACIÓN DE IMÁGENES DE ADS (BAJO COSTO CON IMAGEN 3) ---
      this.logger.log('Extracting ads image prompts and generating low-cost campaign ads images...');
      ...
      this.logger.log(`Found ${matches.length} campaign ads images to generate.`);

      // Sequentially generate images to avoid overloading or rate limits (only 9 campaigns)
      let modifiedMarkdown = unifiedStrategyMarkdown;
      for (let i = 0; i < matches.length; i++) {
        const [originalTag, promptText] = matches[i];
        this.logger.log(`Generating ad image ${i + 1}/${matches.length} with prompt: "${promptText.substring(0, 60)}..."`);
        
        try {
          const imageBuffer = await this.geminiService.generateImage(promptText, 'imagen-3.0-generate-002');
          const imagePath = path.join(tempDir, `ad_campaign_${i + 1}.jpg`);
          fs.writeFileSync(imagePath, imageBuffer);
          this.logger.log(`Ad image successfully saved to: ${imagePath}`);
          
          // Replace tag in Markdown with the image path tag for PdfService to draw it
          modifiedMarkdown = modifiedMarkdown.replace(originalTag, `[IMAGE: ${imagePath}]`);
        } catch (imgErr) {
